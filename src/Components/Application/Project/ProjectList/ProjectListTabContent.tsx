    ...
    <Card>
      <CardBody>
        <TabContent activeTab={activeTab}>
          ...
          <TabPane tabId="5"><Row>{createdFormData.map((item, i) => (item.badge === "BulkEmail" ? <ProjectCommon item={item} key={i} /> : " "))}</Row></TabPane>

          <TabPane tabId="6"><Row>{createdFormData.map((item, i) => (item.badge === "ComplianceFrontiers" ? <ProjectCommon item={item} key={i} /> : " "))}</Row></TabPane>
