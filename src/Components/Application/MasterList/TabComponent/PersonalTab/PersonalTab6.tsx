...
const PersonalTab = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://backend-chess-tau.vercel.app/get_forms_form_Bear_Middletown_Chess_Coaching");
  ...
  const fetchProfileData = async (profileId: string) => {
    try {
      const response = await axios.get(`https://backend-chess-tau.vercel.app/form_Bear_Middletown_Chess_Coaching_by_profile_id`, {
    ...
    if (window.confirm("Are you sure you want to delete this profile?")) {
      try {
        const response = await axios.delete(
          `https://backend-chess-tau.vercel.app/form_Bear_Middletown_Chess_Coaching_bp_delete_records_by_profile_ids`, 
    ...
    <Card>
      <CardHeader>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h4>
           Bear Middletown Chess Coaching List{" "}
            <a
