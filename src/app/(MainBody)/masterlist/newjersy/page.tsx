  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Filter data where New_Jersey_Chess_Tournament is true
        const njTournamentData = data.filter(
          (record: FormRecord) => record.New_Jersey_Chess_Tournament === true
        );
  
        setFormData(njTournamentData); // Set filtered data to state
        setFilteredData(njTournamentData);
        setLoading(false);
      } catch (error) {
